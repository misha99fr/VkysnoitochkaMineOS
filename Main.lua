...
local function requestAccessCode()
    local input = GUI.addInput("Введите код доступа", "")
    if input == "test1" then
        GUI.alert("Доступ разрешен")
        return true
    else
        GUI.alert("Неверный код доступа. Приложение будет закрыто.")
        return false
    end
end
...
local function showWarning()
    GUI.alert("Внимание: Это приложение для менеджера Макдональдс!")
end
...
-- Запрос кода доступа и запуск приложения при правильном коде
if requestAccessCode() then
    createApplication()
end
